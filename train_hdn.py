...
parser.add_argument('--mode', type=str, help='the mode to use')
parser.add_argument('--frequency_baseline', action='store_true')
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(train_set, batch_size=1, shuffle=True, num_workers=1, pin_memory=False)
    ...
    if args.evaluate:
        ...
        for epoch in range(0, args.max_epoch):
            ...
            print(('Epoch[{epoch:d}]:'.format(epoch = epoch)), end=' ') 
            for idx, top_N in enumerate(top_Ns):
                print(('\t[Recall@{top_N:d}] {recall:2.3f}%% (best: {best_recall:2.3f}%%)'.format(
                    top_N=top_N, recall=recall[idx] * 100, best_recall=best_recall[idx] * 100)), end=' ')
    ...
    for i, (im_data, im_info, gt_objects, gt_relationships, gt_regions) in enumerate(train_loader):
        ...
        if  (i + 1) % args.log_interval == 0:
            ...
            print(('\t[Loss]\tobj_cls_loss: %.4f\tobj_box_loss: %.4f' %
                  (train_obj_cls_loss.avg, train_obj_box_loss.avg)), end=' ')
            print(('\tpred_cls_loss: %.4f,' % (train_pred_cls_loss.avg)), end=' ')

            if not args.disable_language_model:
                print(('\tcaption_loss: %.4f,' % (train_region_caption_loss.avg)), end=' ')
            if args.region_bbox_reg:
                print(('\tregion_box_loss: %.4f, ' % (train_region_box_loss.avg)), end=' ')
            print(('\tregion_objectness_loss: %.4f' % (train_region_objectiveness_loss.avg)), end=' ')
...
def test(test_loader, net, top_Ns):
    ...
    for i, (im_data, im_info, gt_objects, gt_relationships, gt_regions) in enumerate(tqdm(test_loader)):
        # Forward pass
        if args.mode == 'sgdet':
            total_cnt_t, rel_cnt_correct_t = net.evaluate(
                im_data, im_info, gt_objects.numpy()[0], gt_relationships.numpy()[0], gt_regions.numpy()[0],
                top_Ns = top_Ns, nms=True, freq_baseline=args.frequency_baseline)
        elif args.mode == 'sgcls':
            total_cnt_t, rel_cnt_correct_t = net.evaluate(
                im_data, im_info, gt_objects.numpy()[0], gt_relationships.numpy()[0], gt_regions.numpy()[0],
                top_Ns = top_Ns, nms=False, use_gt_boxes=True, use_gt_regions=True, freq_baseline=args.frequency_baseline)
        ...
        if (i + 1) % 100 == 0 and i > 0:
            for idx, top_N in enumerate(top_Ns):
                print('\n[%d/%d][Evaluation] Top-%d Recall: %2.3f%%\n' % (
                    i+1, len(test_loader), top_N, rel_cnt_correct[idx] / float(rel_cnt) * 100), flush=True)
